...
DEFAULT_FLOW = (-1, 0, 1, 0)

FlowKey = tuple[int, int, int, int]

# Base sprite stems on disk (8 PNG files).
...
FLOW_SPRITE_SPEC: dict[FlowKey, SpriteSpec] = {
    **{flow: SpriteSpec(stem) for flow, stem in FLOW_TO_SPRITE.items()},
    (0, -1, -1, 0): SpriteSpec("up_right", flip_h=True),
    (-1, 0, 0, 1): SpriteSpec("left_up", flip_v=True),
    (1, 0, 0, -1): SpriteSpec("right_down", flip_v=True),
    (0, 1, 1, 0): SpriteSpec("down_left", flip_h=True),
}
