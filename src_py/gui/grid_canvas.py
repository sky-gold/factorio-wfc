...
def screen_to_cell(
    ...
    x = int(rel_x // CELL_PITCH)
    y = int(rel_y // CELL_PITCH)
    if x < 0 or y < 0 or x >= width or y >= height:
        return -1, -1

    if rel_x - x * CELL_PITCH >= CELL_SIZE or rel_y - y * CELL_PITCH >= CELL_SIZE:
        return -1, -1
    return x, y
...
def _add_lane_overlay(
    ...
    *,
    ...
    dpg.add_image(
        ...
        parent=parent,
        pos=(cell_x * CELL_PITCH + offset_x, cell_y * CELL_PITCH + offset_y),
    )
